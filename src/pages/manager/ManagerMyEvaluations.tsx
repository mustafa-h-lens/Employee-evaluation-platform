...
export const ManagerMyEvaluations: React.FC = () => {
  const { user } = useAuth();
  ...
  const fetchEvaluations = async () => {
    ...
    try {
      const { data } = await supabase
        ...
        .in('status', ['موافقة', 'بانتظار الموافقة', 'مرفوض'])
      ...
      rows.forEach(ev => {
        replies[ev.id] = ev.director_note || '';
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2">عرض التقييمات الخاصة بك من مدير الإدارة</p>
      </div>
