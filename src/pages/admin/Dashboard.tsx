  ...
  const fetchStats = async () => {
    try {
      const [
        ...
        supabase.from('evaluations').select('*', { count: 'exact', head: true }).in('status', ['تم الإرسال', 'اطلع الموظف', 'مغلق']),
      ...
      setStats({
        ...
        activePeriod: activePeriodData ? `الربع ${activePeriodData.quarter} - ${activePeriodData.year}` : 'لا توجد فترة نشطة',
